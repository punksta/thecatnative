...
export const loadKittiesToList = (actions: Observable<Action>, store: *): Observable<Action> => {
	...
	return actions.filter((a: Action) => {
		...
		.mergeMap(a => {
			...
			const requestAction = asObservable(Api.fetchKitties_({count: 20, type: 'gif'}))
				.map(data => ({type: 'KITTY_LIST_SUCCESS', data, refresh}))
				.catch(e => Observable.of({type: 'KITTY_LIST_FAILURE', refresh, meeeooow: e.toString()}));
